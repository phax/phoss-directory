 ...
 * where graceful retries will happen.<br>
 * Each collector runs in a virtual thread, because handling a work item is dominated by waiting for
 * the SMP to respond and not by CPU usage. The number of collectors is therefore the number of SMP
 * queries performed in parallel, and it may be raised way beyond the number of available cores -
 * see the configuration property <code>indexer.maxparallel</code>.
 *
 * @author Philip Helger
 */
public final class IndexerWorkItemQueue
{
  private final int m_nMaxParallel;
  private final LinkedBlockingQueue <Object> m_aQueue;
  private final ThreadFactory m_aThreadFactory = Thread.ofVirtual ().name ("pd-indexer-", 0).factory ();
  private final ExecutorService m_aSenderThreadPool = Executors.newThreadPerTaskExecutor (m_aThreadFactory);
   ...
   * @param nMaxParallel
   *        The number of work items to be handled in parallel. Must be &gt; 0.
   */
  public IndexerWorkItemQueue (@NonNull final IConcurrentPerformer <IIndexerWorkItem> aPerformer,
                               @Nonnegative final int nMaxParallel)
  {
    ValueEnforcer.notNull (aPerformer, "Performer");
    ValueEnforcer.isGT0 (nMaxParallel, "MaxParallel");
    m_nMaxParallel = nMaxParallel;
    m_aImmediateCollector = GenericReflection.uncheckedCast (new ConcurrentCollectorSingle [nMaxParallel]);
  ...
  public ICommonsList <IIndexerWorkItem> stop ()
  {
    ...
    for (int i = 0; i < m_nMaxParallel; ++i)
